...
services:
  app:
    ...
    environment:
      ...
      - DATABASE_URL=postgresql://wagateway:wagateway@postgres:5432/wagateway
...
services:
  app:
    image: ghcr.io/HarishBoke/wagateway:latest
    deploy:
      replicas: 1
      resources:
        limits:
...
# DATABASE_URL=postgresql://user:pass@localhost:5432/wagateway
    ...
    rules:
      ...
      - alert: ServiceDown
        ...
        labels:
          severity: critical
        annotations:
          summary: "WA Gateway service is down"
          description: "The WA Gateway application is not responding"
```
...
receivers:
  - name: 'slack-notifications'
    slack_configs:
      - channel: '#wagateway-alerts'
        send_resolved: true

  - name: 'slack-critical'
    slack_configs:
      - channel: '#wagateway-critical'
        send_resolved: true
...
```json
// monitoring/grafana/dashboards/wagateway.json
{
  "title": "WA Gateway Dashboard",
  "uid": "wagateway-main",
  "panels": [
    {
...
BACKUP_DIR="/backups"
S3_BUCKET="s3://wagateway-backups"
...
# Download from S3
aws s3 cp s3://wagateway-backups/database/db_$BACKUP_DATE.dump.gz /tmp/
aws s3 cp s3://wagateway-backups/sessions/sessions_$BACKUP_DATE.tar.gz /tmp/
