...
WA Gateway provides an idempotency mechanism to prevent duplicate processing on the client side.
...
| `X-WA Gateway-Delivery-Id` | Header carrying the delivery ID |
| `X-WA Gateway-Idempotency-Key` | Header carrying the idempotency key |
| `X-WA Gateway-Retry-Count` | Retry attempt number (0 = first attempt) |
...
async function handleWebhook(req: Request, res: Response) {
  const idempotencyKey = req.headers['x-wagateway-idempotency-key'] as string;
  const retryCount = parseInt(req.headers['x-wagateway-retry-count'] as string) || 0;
...
app.post('/webhook', (req, res) => {
  const signature = req.headers['x-wagateway-signature'];
...
In addition to the REST API, WA Gateway provides WebSocket for real-time updates.
...
|----------|---------|--------|
| JavaScript/TypeScript | `@wagateway/sdk` | 🔜 Planned |
| Python | `wagateway-python` | 🔜 Planned |
| PHP | `wagateway/php-sdk` | 🔜 Planned |
| Go | `wagateway-go` | 🔜 Planned |
...
# Usage
client = WA Gateway('http://localhost:2785/api', 'your-api-key')
