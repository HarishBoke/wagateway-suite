...
### Adapter
An interface implementation that provides a specific capability. In WA Gateway, adapters are used for:
...
### API Key
Authentication token to access the WA Gateway API. Sent via the `X-API-Key` header.
...
### Chrome/Chromium
Browser used by Puppeteer to run WhatsApp Web. WA Gateway uses headless Chromium.
...
### Dashboard
Web interface to manage WA Gateway without using the API directly. Built with React + shadcn/ui.

### Dead Letter Queue (DLQ)
Queue that stores messages that failed after all retry attempts. Used for debugging and manual retry.

### Docker
Containerization platform for packaging and deploying applications. WA Gateway is distributed as a Docker image.
...
### GHCR
GitHub Container Registry - registry for storing Docker images. The WA Gateway image is available at `ghcr.io/HarishBoke/wagateway`.
...
### Multi-session
Ability to run multiple WhatsApp sessions within a single WA Gateway instance.

## N

### NestJS
Node.js framework for building server-side applications. The WA Gateway backend is built with NestJS.

### Node.js
JavaScript runtime used to run WA Gateway. Supported version: Node.js 20 LTS.

## O

### ORM (Object-Relational Mapping)
Library that maps objects in code to database tables. WA Gateway uses TypeORM.
...
### Plugin
Extension that can be added to WA Gateway to add functionality without modifying the core codebase.
...
### REST API
Architectural style for APIs used by WA Gateway. Uses HTTP methods (GET, POST, PUT, DELETE).
...
AWS object storage service. WA Gateway supports S3-compatible storage for media files.
...
### Swagger
API documentation tool. WA Gateway provides Swagger UI at `/api/docs`.
...
### TypeScript
Typed superset of JavaScript used for WA Gateway development.
...
### Volume (Docker)
Persistent storage for Docker containers. WA Gateway data is stored in volumes.

## W

### WAHA
WhatsApp HTTP API - a similar project that inspired WA Gateway. WA Gateway is built as an open-source alternative.
...
### whatsapp-web.js
Primary Node.js library used by WA Gateway to interact with WhatsApp Web. Uses Puppeteer to control the browser.
