...
This document provides a complete collection of WA Gateway API endpoints with request/response examples for each endpoint. It can be used as a quick reference or imported into tools such as Postman, Insomnia, or Bruno.
...
**Response:**
```
# HELP wagateway_sessions_total Total number of sessions
# TYPE wagateway_sessions_total gauge
wagateway_sessions_total{status="connected"} 4
wagateway_sessions_total{status="disconnected"} 1

# HELP wagateway_messages_total Total messages processed
# TYPE wagateway_messages_total counter
wagateway_messages_total{direction="incoming"} 15234
wagateway_messages_total{direction="outgoing"} 8567

# HELP wagateway_api_requests_total Total API requests
# TYPE wagateway_api_requests_total counter
wagateway_api_requests_total{method="GET",status="200"} 45678
wagateway_api_requests_total{method="POST",status="200"} 12345
```
...
```json
{
  "info": {
    "name": "WA Gateway API",
    "description": "WA Gateway Messaging API Gateway",
...
Download: [wagateway-postman-collection.json](./assets/wagateway-postman-collection.json)

## 07.10 cURL Examples Collection

```bash
#!/bin/bash
# wagateway-curl-examples.sh
