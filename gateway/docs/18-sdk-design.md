...
WA Gateway provides official SDKs for multiple programming languages to simplify API integration. This document describes the SDK design and specifications.
...
```bash
# npm
npm install @wagateway/sdk

# yarn
yarn add @wagateway/sdk

# pnpm
pnpm add @wagateway/sdk
```

### Quick Start

```typescript
import { WA Gateway } from '@wagateway/sdk';

const client = new WA Gateway({
...
export interface WA GatewayConfig {
  baseUrl: string;
  apiKey: string;
  timeout?: number;
  retries?: number;
...
export class WA Gateway extends EventEmitter {
  private config: WA GatewayConfig;
  ...
  constructor(config: WA GatewayConfig) {
    super();
...
export interface WA GatewayEvents {
  // Session events
...
export class WA GatewayError extends Error {
  constructor(
    message: string,
    public code: string,
    ...
    super(message);
    this.name = 'WA GatewayError';
  }
}

export class ValidationError extends WA GatewayError {
...
export class AuthenticationError extends WA GatewayError {
...
export class SessionNotFoundError extends WA GatewayError {
...
export class RateLimitError extends WA GatewayError {
...
```bash
pip install wagateway-sdk
```

### Quick Start

```python
from wagateway import WA Gateway
import asyncio
...
class SessionsResource:
    def __init__(self, client: WA Gateway):
        self.client = client
...
class WA GatewayError(Exception):
...
from .async_client import WA Gateway as AsyncWA Gateway
import asyncio

class WA GatewaySync:
    """Synchronous wrapper for WA Gateway client"""

    def __init__(self, **kwargs):
        self._async_client = AsyncWA Gateway(**kwargs)
...
# Usage
from wagateway.sync import WA GatewaySync

client = WA GatewaySync(base_url="http://localhost:2785", api_key="your-key")
...
```bash
composer require wagateway/sdk
```
...
use WA Gateway\Client;
use WA Gateway\Resources\Messages;
...
use WA Gateway\Resources\Sessions;
use WA Gateway\Resources\Messages;
use WA Gateway\Resources\Contacts;
use WA Gateway\Resources\Groups;
use WA Gateway\Resources\Webhooks;
use WA Gateway\Exceptions\WA GatewayException;
...
namespace WA Gateway\Resources;

use WA Gateway\Client;
use WA Gateway\Models\Session;
...
```typescript
// nodes/WA Gateway/WA Gateway.node.ts

import { IExecuteFunctions, INodeExecutionData, INodeType, INodeTypeDescription } from 'n8n-workflow';

export class WA Gateway implements INodeType {
  description: INodeTypeDescription = {
    ...
    description: 'Send WhatsApp messages via WA Gateway',
    defaults: {
      name: 'WA Gateway',
    },
    inputs: ['main'],
...
export class WA GatewayTrigger implements INodeType {
  description: INodeTypeDescription = {
    ...
    group: ['trigger'],
    version: 1,
    description: 'Starts workflow on WA Gateway events',
    defaults: {
      name: 'WA Gateway Trigger',
    },
    inputs: [],
    outputs: ['main'],
    credentials: [
  ...
  webhookMethods = {
    default: {
      async checkExists(this: IHookFunctions): Promise<boolean> {
        // Check if webhook already exists in WA Gateway
