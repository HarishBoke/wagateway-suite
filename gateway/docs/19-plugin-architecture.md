...
|-----------|--------|-------|
| **@wagateway/plugin-sdk** | 🔜 Planned | NPM package not yet published |
...
The plugin architecture enables WA Gateway extensibility without modifying the core codebase. Plugins can add new features, integrate with external services, or customize behavior.
...
```json
{
  ...
  "version": "1.0.0",
  "description": "An awesome plugin for WA Gateway",
  "author": "Your Name",
  "license": "MIT",
...
import { WA GatewayPlugin, PluginContext, MessageEvent, HookResult } from '@wagateway/plugin-sdk';
...
export default class MyAwesomePlugin implements WA GatewayPlugin<MyPluginConfig> {
  name = 'my-awesome-plugin';
  version = '1.0.0';
...
```typescript
// @wagateway/plugin-sdk/types.ts

export interface WA GatewayPlugin<TConfig = any> {
  name: string;
  version: string;
...
```typescript
// @wagateway/plugin-sdk/storage.ts
...
```mermaid
sequenceDiagram
    participant Core as WA Gateway Core
...
import { WA GatewayPlugin, PluginContext, PluginManifest } from './types';
...
import { WA GatewayPlugin, PluginContext, MessageEvent } from '@wagateway/plugin-sdk';
...
export default class AutoReplyPlugin implements WA GatewayPlugin<AutoReplyConfig> {
  name = 'auto-reply';
  version = '1.0.0';
...
export default class TranslationPlugin implements WA GatewayPlugin<TranslationConfig> {
  name = 'translation';
  version = '1.0.0';
