...
WA Gateway uses a database to store:
...
WA Gateway supports two database backends that can be selected at deployment time:
...
WA Gateway v0.2+ implements a **dual-database architecture** that separates boot configuration from user data:

```
┌─────────────────────────────────────────────────────────────────┐
│                        WA Gateway Application                        │
├─────────────────────────────┬───────────────────────────────────┤
...
When using PostgreSQL Built-in mode, WA Gateway automatically:
...
WA Gateway provides endpoints for migrating data between database types:
...
To ensure date/time values work across both SQLite and PostgreSQL, WA Gateway uses a `DateTransformer` that stores dates as ISO 8601 text strings:
...
# Create backup
pg_dump -Fc $DB_NAME > $BACKUP_DIR/wagateway_$DATE.dump

# Compress
gzip $BACKUP_DIR/wagateway_$DATE.dump

# Upload to S3 (optional)
aws s3 cp $BACKUP_DIR/wagateway_$DATE.dump.gz s3://backups/wagateway/
