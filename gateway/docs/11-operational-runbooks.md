...
This document contains Standard Operating Procedures (SOP) for WA Gateway operations, including incident response, maintenance procedures, and troubleshooting guides.
...
docker compose logs --tail=100 wagateway
...
# A. Container crashed
docker compose logs wagateway 2>&1 | grep -i "error\|fatal\|crash"

# B. Out of memory
docker compose logs wagateway 2>&1 | grep -i "oom\|memory"

# C. Database connection
docker compose logs wagateway 2>&1 | grep -i "database\|connection refused"

# 5. Apply fix based on cause:

# A. Simple restart
docker compose restart wagateway
...
docker compose logs wagateway 2>&1 | grep -i "{sessionId}" | tail -20
...
docker stats --no-stream wagateway
free -m
...
docker compose logs wagateway 2>&1 | grep -i "heap\|memory\|gc"
...
curl -v https://your-webhook-endpoint.com/wagateway

# B. SSL certificate issues
curl -v --insecure https://your-webhook-endpoint.com/wagateway

# C. Timeout
curl -v --max-time 30 https://your-webhook-endpoint.com/wagateway

# D. Authentication failed
curl -v -H "Authorization: Bearer token" \
  https://your-webhook-endpoint.com/wagateway
...
# Verify backup
ls -la /backups/wagateway/$(date +%Y%m%d)/
...
# 2. Create backup
./scripts/backup.sh
BACKUP_DIR="/backups/wagateway/$(date +%Y%m%d-%H%M%S)"

# 3. Export current state
docker compose exec wagateway npm run export -- --output /tmp/export.json
docker cp wagateway:/tmp/export.json $BACKUP_DIR/

# 4. Stop services
docker compose down

# 5. Update version in docker-compose.yml
# Change: image: ghcr.io/HarishBoke/wagateway:0.1.0
# To:     image: ghcr.io/HarishBoke/wagateway:0.2.0

# 6. Pull new image
docker compose pull

# 7. Run database migrations (if any)
docker compose run --rm wagateway npm run migration:run
...
# 3. Restore database
cp $BACKUP_DIR/wagateway.db ./data/
...
BACKUP_DIR="/backups/wagateway/$DATE"
RETENTION_DAYS=30
...
if [ "$DATABASE_ADAPTER" = "postgresql" ]; then
    ...
    docker compose exec -T postgres pg_dump -U postgres wagateway > "$BACKUP_DIR/database.sql"
else
    echo "Backing up SQLite..."
    # Use SQLite online backup
    docker compose exec wagateway sqlite3 /app/data/wagateway.db ".backup /tmp/backup.db"
    docker cp wagateway:/tmp/backup.db "$BACKUP_DIR/wagateway.db"
fi
...
tar -czf "$BACKUP_DIR.tar.gz" -C "/backups/wagateway" "$DATE"
rm -rf "$BACKUP_DIR"

# 6. Cleanup old backups
echo "Cleaning up old backups..."
find /backups/wagateway -name "*.tar.gz" -mtime +$RETENTION_DAYS -delete
...
if [ -f "$RESTORE_DIR/*/database.sql" ]; then
    ...
    sleep 10
    docker compose exec -T postgres psql -U postgres wagateway < "$RESTORE_DIR"/*/database.sql
elif [ -f "$RESTORE_DIR/*/wagateway.db" ]; then
    echo "Restoring SQLite..."
    cp "$RESTORE_DIR"/*/wagateway.db ./data/
fi
...
truncate -s 0 ./logs/wagateway.log
