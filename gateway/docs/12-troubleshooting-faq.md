...
docker compose logs wagateway 2>&1 | grep -i "whatsapp\|puppeteer\|browser"
...
```bash
# Check memory usage
docker stats wagateway --no-stream
...
sqlite3 ./data/wagateway.db ".timeout 30000"

# Increase timeout in configuration
DATABASE_SQLITE_BUSY_TIMEOUT=30000

# Check WAL mode
sqlite3 ./data/wagateway.db "PRAGMA journal_mode;"
# Should return: wal

# Enable WAL mode
sqlite3 ./data/wagateway.db "PRAGMA journal_mode=WAL;"
```
...
docker exec wagateway ping postgres
docker exec wagateway curl http://host.docker.internal:8080
```
...
**Q: Is WA Gateway safe to use?**
> A: WA Gateway uses unofficial WhatsApp Web API. While we implement best practices to avoid detection, there's inherent risk of account restrictions. We recommend:
...
> A: Yes, WA Gateway works with both personal and WhatsApp Business accounts. Note that WhatsApp Business API (official Meta API) is different and not supported.
...
> 2. Set URL: `http://wagateway:2785/api/sessions/{id}/messages`
...
if [ "$DATABASE_ADAPTER" = "postgresql" ]; then
    ...
    cp ./data/wagateway.db "$BACKUP_DIR/$DATE/"
fi
...
2. **Check logs** - `docker compose logs wagateway --tail=100`
...
- **GitHub Issues**: [github.com/HarishBoke/wagateway-suite/issues](https://github.com/HarishBoke/wagateway-suite/issues)
- **Discussions**: [github.com/HarishBoke/wagateway-suite/discussions](https://github.com/HarishBoke/wagateway-suite/discussions)
- **Discord**: [discord.gg/wagateway](https://discord.gg/wagateway) (if available)
- **Stack Overflow**: Tag with `wagateway`
---
