...
flowchart TB
    ...
    subgraph WA Gateway["WA Gateway Platform"]
...
WA Gateway is designed with a **Pluggable Architecture** that allows infrastructure components to be swapped without changing application code. This enables flexible deployments ranging from minimal single-session bots to enterprise-scale multi-tenant platforms.
...
WA Gateway uses NestJS Dynamic Modules for adapter injection:
...
WA Gateway uses the adapter pattern for infrastructure components that can be swapped per deployment needs. This allows users with limited resources to run WA Gateway without heavyweight external dependencies.

### Adapter Overview

```mermaid
flowchart TB
    subgraph Core["WA Gateway Core"]
...
WA Gateway supports SQLite for lightweight deployments and PostgreSQL for high-volume production.
...
export const getDatabaseConfig = (config: ConfigService): TypeOrmModuleOptions => {
  ...
  if (dbType === 'sqlite') {
    return {
      ...baseConfig,
      type: 'sqlite',
      database: config.get('database.sqlite.path', './data/wagateway.db'),
...
WA Gateway provides several deployment profiles for different needs:
...
DATABASE_SQLITE_PATH=./data/wagateway.db
...
# Database
DATABASE_TYPE=postgres
DATABASE_URL=postgresql://wagateway:password@db-cluster:5432/wagateway
DATABASE_POOL_MAX=50

# Storage
STORAGE_TYPE=s3
STORAGE_S3_BUCKET=wagateway-media
