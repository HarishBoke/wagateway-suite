...
This guide explains how to deploy WA Gateway in a horizontally scaled environment for high availability and increased capacity.
...
flowchart TB
    ...
    subgraph Nodes["WA Gateway Nodes"]
        N1[WA Gateway Node 1]
        N2[WA Gateway Node 2]
        N3[WA Gateway Node 3]
    end
...
services:
  wagateway:
    image: ghcr.io/HarishBoke/wagateway:0.2.0
    deploy:
      replicas: 3
      update_config:
        parallelism: 1
    ...
    environment:
      ...
      - DATABASE_NAME=wagateway
      - DATABASE_USER=wagateway
...
# Deploy stack
docker stack deploy -c docker-compose.swarm.yml wagateway

# Scale up/down
docker service scale wagateway_wagateway=5

# Check status
docker service ls
docker service ps wagateway_wagateway
```
...
kind: Ingress
metadata:
  name: wagateway-ingress
  namespace: wagateway
  annotations:
    nginx.ingress.kubernetes.io/affinity: 'cookie'
    nginx.ingress.kubernetes.io/session-cookie-name: 'wagateway-session'
    nginx.ingress.kubernetes.io/session-cookie-max-age: '172800'
spec:
  ingressClassName: nginx
  rules:
    - host: wagateway.example.com
      http:
        paths:
          - path: /
            pathType: Prefix
  ...
  tls:
    - hosts:
        - wagateway.example.com
      secretName: wagateway-tls
```
...
# Check pods
kubectl get pods -n wagateway

# Check logs
kubectl logs -f deployment/wagateway -n wagateway

# Scale
kubectl scale statefulset wagateway --replicas=5 -n wagateway
```
...
http:
  routers:
    wagateway:
      rule: 'Host(`wagateway.example.com`)'
      service: wagateway
      middlewares:
        - sticky-session
      ...
      loadBalancer:
        sticky:
          cookie:
            ...
            httpOnly: true
        servers:
          - url: 'http://wagateway-1:2785'
          - url: 'http://wagateway-2:2785'
          - url: 'http://wagateway-3:2785'
        healthCheck:
          path: /api/health
          interval: 10s
          timeout: 3s
...
upstream wagateway {
    ...
    server wagateway-1:2785 weight=1 max_fails=3 fail_timeout=30s;
    server wagateway-2:2785 weight=1 max_fails=3 fail_timeout=30s;
    server wagateway-3:2785 weight=1 max_fails=3 fail_timeout=30s;
}

server {
    listen 80;
    server_name wagateway.example.com;

    location / {
        proxy_pass http://wagateway;
...
```yaml
# prometheus/wagateway-rules.yaml
groups:
  - name: wagateway
    rules:
      - alert: HighMemoryUsage
        expr: container_memory_usage_bytes{container="wagateway"} > 1.8e9
        for: 5m
        labels:
          severity: warning
        annotations:
          summary: 'WA Gateway node high memory usage'

      - alert: NodeDown
        expr: up{job="wagateway"} == 0
        for: 1m
        labels:
          severity: critical
        annotations:
          summary: 'WA Gateway node is down'
```
