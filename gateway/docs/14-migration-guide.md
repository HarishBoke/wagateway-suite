...
This document provides a comprehensive guide for migrating WA Gateway, including:
...
WA Gateway v0.2+ includes built-in migration API endpoints that leverage the **Dual-Database Architecture**:
...
>
> WA Gateway separates databases:
>
...
WA Gateway v0.2+ supports migrating media files between storage backends:
...
Redis in WA Gateway is used **only for caching** with TTL-based expiration. Cache data is ephemeral and automatically regenerates from the database.
...
const config: MigrationConfig = {
  sqlitePath: process.env.SQLITE_PATH || './data/wagateway.db',
  postgresUrl: process.env.DATABASE_URL || 'postgresql://user:pass@localhost:5432/wagateway',
...
```bash
# Step 1: Stop WA Gateway
docker compose down

# Step 2: Backup current data
cp -r ./data ./data-backup-$(date +%Y%m%d)
docker exec wagateway-db pg_dump -U postgres wagateway > backup.sql
...
export DATABASE_URL=postgresql://user:pass@localhost:5432/wagateway
...
ssh old-server "sqlite3 /data/wagateway.db \
    \"SELECT * FROM sessions WHERE id='${SESSION_ID}'\" \
    -csv" > session_record.csv

# Import to new database
echo "📥 Importing session record..."
ssh new-server "sqlite3 /data/wagateway.db \
...
echo "🚀 Upgrading WA Gateway v0.1.x → v0.2.x"
...
docker run --rm \
  -v $(pwd)/data:/app/data \
  -e DATABASE_URL=sqlite:///app/data/wagateway.db \
  ghcr.io/HarishBoke/wagateway:0.2.0 \
  npm run migration:run

# 4. Migrate configuration
echo "⚙️ Migrating configuration..."
cat > .env.new << 'EOF'
# WA Gateway v0.2.x Configuration

# Database (unchanged if using SQLite)
DATABASE_ADAPTER=sqlite
DATABASE_URL=sqlite:./data/wagateway.db
...
docker exec wagateway npm run cli -- create-api-key --name "migrated-key"
...
echo "🚀 Upgrading WA Gateway v0.2.x → v1.0.0"

# Pre-flight checks
CURRENT_VERSION=$(docker inspect ghcr.io/HarishBoke/wagateway --format '{{.Config.Labels.version}}' 2>/dev/null || echo "unknown")
...
if [ "$DATABASE_ADAPTER" = "postgresql" ]; then
  ...
  cp ./data/wagateway.db "$BACKUP_DIR/"
fi
...
docker run --rm \
  -v $(pwd)/data:/app/data \
  --env-file .env \
  ghcr.io/HarishBoke/wagateway:1.0.0 \
  npm run migration:run
    ...
    - name: Configure staging webhooks
      command: |
        npm run cli -- set-webhook \
          --url https://staging-webhook.example.com/wagateway \
...
sqlite3 ./data/wagateway.db "PRAGMA integrity_check;"

# Verify auth session files
ls -la ./data/.wwebjs_auth/session-*/

# Check file permissions
stat ./data/wagateway.db
