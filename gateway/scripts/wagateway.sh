#!/bin/bash
# WA Gateway Smart Orchestration Script
...
cmd_start() {
    ...
    echo ""
    log_success "WA Gateway started successfully!"
    echo ""
...
# Show status
cmd_status() {
    log_info "WA Gateway container status:"
    echo ""
    cd "$PROJECT_DIR"
...
# Build images
cmd_build() {
    log_info "Building WA Gateway images..."
    load_env
    local profiles=$(get_profiles)
    cd "$PROJECT_DIR"
...
cmd_help() {
    echo ""
    echo "WA Gateway Smart Orchestration Script"
    echo ""
    echo "Usage: $0 <command> [options]"
    echo ""
    echo "Commands:"
    echo "  start       Start WA Gateway with auto-detected profiles"
    echo "  stop        Stop all WA Gateway containers"
    echo "  restart     Restart WA Gateway"
    echo "  status      Show container status"
    echo "  logs        Show logs (default: wagateway-api)"
