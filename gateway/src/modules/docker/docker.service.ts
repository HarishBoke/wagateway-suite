...
export class DockerService implements OnModuleInit {
  ...
  /**
   * List all WA Gateway-related containers
   */
  async listContainers(): Promise<ContainerInfo[]> {
    ...
    try {
      ...
      return containers
        .filter(c => {
          // Filter by WA Gateway labels or name prefix
          const labels = c.Labels || {};
          return labels['com.wagateway.service'] || c.Names?.some(n => n.startsWith('/wagateway-'));
        })
        .map(c => ({
  ...
  async getContainerByService(service: string): Promise<Docker.Container | null> {
    ...
    try {
      const containers = await this.docker.listContainers({
        all: true,
        filters: {
          label: [`com.wagateway.service=${service}`],
        },
      });
      ...
      const match = allContainers.find(c => c.Names?.some(n => n.includes(`wagateway-${service}`) || n.includes(service)));
  ...
  private getContainerSpec(profile: string): {
    ...
    const specs: Record<string, ReturnType<typeof this.getContainerSpec>> = {
      redis: {
        ...
        name: 'wagateway-redis',
        alias: 'redis', // DNS alias for resolution
        cmd: ['redis-server', '--appendonly', 'yes'],
        volumes: [{ name: 'wagateway_redis-data', path: '/data' }],
        healthcheck: {
        ...
        labels: {
          'com.wagateway.service': 'cache',
          'com.wagateway.builtin': 'true',
        },
      },
      postgres: {
        image: 'postgres:16-alpine',
        name: 'wagateway-postgres',
        alias: 'postgres',
        // Use hardcoded defaults for built-in container (don't inherit SQLite paths)
        env: ['POSTGRES_USER=wagateway', 'POSTGRES_PASSWORD=wagateway', 'POSTGRES_DB=wagateway'],
        volumes: [{ name: 'wagateway_postgres-data', path: '/var/lib/postgresql/data' }],
        healthcheck: {
          test: ['CMD-SHELL', 'pg_isready -U wagateway'],
          interval: 5000000000,
          timeout: 3000000000,
          retries: 5,
        },
        labels: {
          'com.wagateway.service': 'database',
          'com.wagateway.builtin': 'true',
        },
      },
      minio: {
        image: 'minio/minio',
        name: 'wagateway-minio',
        alias: 'minio',
        ...
        volumes: [{ name: 'wagateway_minio-data', path: '/data' }],
        ports: [
