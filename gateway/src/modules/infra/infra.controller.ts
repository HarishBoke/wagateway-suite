...
export class InfraController {
  ...
  saveConfig(@Body() config: SaveConfigDto): { message: string; saved: boolean; envPath: string; profiles: string[] } {
    try {
      ...
      // Header
      envLines.push('# WA Gateway Configuration');
      ...
      if (config.database) {
        ...
        if (config.database.type === 'postgres') {
          if (config.database.builtIn) {
            ...
            envLines.push('DATABASE_USERNAME=wagateway');
            envLines.push('DATABASE_PASSWORD=wagateway');
            envLines.push('DATABASE_NAME=wagateway');
            ...
            envLines.push(`DATABASE_NAME=${config.database.database || 'wagateway'}`);
          }
      ...
      if (config.storage) {
        ...
        if (config.storage.type === 'local') {
          ...
          if (config.storage.builtIn) {
            ...
            envLines.push('S3_BUCKET=wagateway');
