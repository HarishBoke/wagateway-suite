...
export class WebhookService {
  ...
  async test(sessionId: string, webhookId: string): Promise<{ success: boolean; statusCode?: number; error?: string }> {
    ...
    const headers: Record<string, string> = {
      ...
      'User-Agent': 'WA Gateway-Webhook/1.0.0',
      'X-WA Gateway-Event': 'test',
      'X-WA Gateway-Idempotency-Key': testPayload.idempotencyKey,
      'X-WA Gateway-Delivery-Id': testPayload.deliveryId,
      'X-WA Gateway-Retry-Count': '0',
      ...webhook.headers,
    };

    if (webhook.secret) {
      headers['X-WA Gateway-Signature'] = this.generateSignature(body, webhook.secret);
    }
  ...
  async dispatch(sessionId: string, event: string, data: Record<string, unknown>): Promise<void> {
    ...
    for (const webhook of matchingWebhooks) {
      ...
      const headers: Record<string, string> = {
        'Content-Type': 'application/json',
        'User-Agent': 'WA Gateway-Webhook/1.0.0',
        'X-WA Gateway-Event': event,
        'X-WA Gateway-Idempotency-Key': idempotencyKey,
        'X-WA Gateway-Delivery-Id': deliveryId,
        'X-WA Gateway-Retry-Count': '0',
        ...webhook.headers,
      };
      ...
      if (this.queueEnabled && this.webhookQueue) {
        ...
        if (webhook.secret) {
          headers['X-WA Gateway-Signature'] = signature;
        }
  ...
  private async deliverWebhook(
    ...
    headers['X-WA Gateway-Retry-Count'] = String(attempt - 1);

    // Add signature if secret is configured and not already present
    if (webhook.secret && !headers['X-WA Gateway-Signature']) {
      headers['X-WA Gateway-Signature'] = this.generateSignature(body, webhook.secret);
    }
