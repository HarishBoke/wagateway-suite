/**
 * WA Gateway JavaScript/TypeScript SDK
 *
 * Official client library for the WA Gateway Messaging API Gateway.
 *
 * @example
 * ```typescript
...
export interface WA GatewayClientConfig {
  /** Base URL of the WA Gateway API (e.g., 'http://localhost:2785') */
  baseUrl: string;
...
export class WA GatewayClient {
  private readonly config: Required<WA GatewayClientConfig>;

  constructor(config: WA GatewayClientConfig) {
    this.config = {
      timeout: 30000,
      ...config,
    };
  ...
  private async request<T>(method: string, path: string, body?: unknown): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`WA Gateway API Error (${response.status}): ${(error as { message: string }).message}`);
    }
...
export default WA GatewayClient;
